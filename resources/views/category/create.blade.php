        ...
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg dark:bg-gray-800">
            <div class="max-w-xl">
                <section>
                    ...
                    <form method="POST" action="/category" class="mt-6 space-y-6">
                        @csrf
                        <div class="space-y-2">
                            <x-form.label for="category_name" :value="__('Nama kriteria')"/>
                            <x-form.input id="category_name" name="category_name" type="text" class="block w-full" :value="old('category_name')" required autofocus autocomplete="category_name"/>
                            <x-form.error :messages="$errors->get('category_name')" />
                        </div>

                        <div class="space-y-2">
                            <x-form.label for="weight" :value="__('Bobot')"/>
                            <x-form.input id="weight" name="weight" type="text" class="block w-full" :value="old('weight')" required autofocus autocomplete="weight"/>
