                ...
                <section>
                    <header>
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            {{ __("Update data peserta disini.") }}
                        </p>
                    </header>

                    <form method="POST" action="/participant/{{ $participant->id }}" class="mt-6 space-y-6">
                        @method('put')
                        @csrf
                        <div class="space-y-2">
                            <x-form.label for="registration_number" :value="__('Nomor Peserta')" />
                            <x-form.input id="registration_number" name="registration_number" type="text" class="block w-full" :value="old('registration_number', $participant->registration_number)" required autofocus autocomplete="registration_number"/>
                            <x-form.error :messages="$errors->get('registration_number')" />
                        </div>

                        <div class="space-y-2">
                            <x-form.label for="name" :value="__('Nama Peserta')"/>
                            <x-form.input id="name" name="name" type="text" class="block w-full" :value="old('name', $participant->name)" required autofocus autocomplete="name"/>                
                        ...
                        <div class="space-y-2">
                            ...
                            <x-form.input id="school" name="school" type="text" class="block w-full" :value="old('school', $participant->school)" required autofocus autocomplete="school"/>                
                            <x-form.error :messages="$errors->get('school')" />
                        </div>

                        <div class="space-y-2">
                            <x-form.label for="birth_place" :value="__('Tempat Lahir')"/>
                            <x-form.input id="birth_place" name="birth_place" type="text" class="block w-full" :value="old('birth_place', $participant->birth_place)" required autofocus autocomplete="birth_place"/>                
                            <x-form.error :messages="$errors->get('birth_place')" />
                        </div>

                        <div class="space-y-2">
                            <x-form.label for="birth_date" :value="__('Tanggal Lahir')"/>
                            <x-form.input id="birth_date" name="birth_date" type="date" class="block w-full" :value="old('birth_date', $participant->birth_date)" required autocomplete="birth_date"/>            
