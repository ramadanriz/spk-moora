        ...
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg dark:bg-gray-800">
            <div class="max-w-xl">
                <section>
                    ...
                    @dd($student->student_id_number)
                    <form method="POST" action="/member-registration/{{ $student->id }}" class="mt-6 space-y-6">
                        @method('put')
                        @csrf
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <x-form.input id="knowledge" name="knowledge" type="number" class="block w-full" :value="old('knowledge', $student->knowledge)" required autocomplete="knowledge"/>            
                            <x-form.error :messages="$errors->get('knowledge')" />
                        </div>

                        <div class="space-y-2">
                            <x-form.label for="interview" :value="__('Nilai Wawancara')"/>
                            <x-form.input id="interview" name="interview" type="number" class="block w-full" :value="old('interview', $student->interview)" required autocomplete="interview"/>            
                            <x-form.error :messages="$errors->get('interview')" />
                        </div>

                        <div class="space-y-2">
                            <x-form.label for="pbb" :value="__('Nilai PBB')"/>
                            <x-form.input id="pbb" name="pbb" type="number" class="block w-full" :value="old('pbb', $student->pbb)" required autocomplete="pbb"/>            
